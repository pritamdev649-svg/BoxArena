/**
 * =========================================================================
 * BoxArena — MongoDB / Mongoose Schema Contract  (v2, production-grade)
 * =========================================================================
 *
 * READ THIS BEFORE GENERATING ANY CODE.
 *
 * ── MONEY RULES (non-negotiable) ────────────────────────────────────────
 * 1. ALL money is stored as INTEGER PAISE. Never floats, never rupees.
 *    ₹250.50 => 25050. Divide by 100 only at the UI render boundary.
 *    Rationale: IEEE-754 cannot represent 0.1; a wallet that drifts by
 *    ₹0.01 per txn is a legal liability in a real-money product.
 * 2. `User.wallet.*` balances are a CACHE. The `Transaction` collection is
 *    the source of truth. Every balance mutation MUST write a Transaction
 *    row inside the same MongoDB session, and MUST stamp `balanceAfterPaise`.
 *    A nightly reconciliation job re-sums the ledger and alarms on drift.
 * 3. Three-bucket wallet (required by Indian RMG norms):
 *      depositPaise  — money the user added. Refundable, NOT winnings.
 *      winningsPaise — money won. Withdrawable to bank (TDS applies).
 *      bonusPaise    — promotional. Playable, NEVER withdrawable.
 *    Debit order for entry fees: bonus -> deposit -> winnings.
 *    Withdrawal is allowed from `winningsPaise` ONLY.
 *
 * ── ID RULES ────────────────────────────────────────────────────────────
 * Every externally-visible entity also carries a short human-safe
 * `publicId` (nanoid, 12 chars). Never expose raw ObjectIds in URLs,
 * WhatsApp invites, or QR codes — they leak collection size + insert time.
 *
 * ── SOFT DELETE ─────────────────────────────────────────────────────────
 * Nothing financial is ever hard-deleted. Use `isActive` / `deletedAt`.
 *
 * See docs/edge_cases.md for the failure modes each field defends against.
 */
 ...
 * service-layer edit that would silently break invariant I1. Corrections are
...
/**
 * Seed these keys on first boot:
 *   paid_challenges_enabled        : boolean  (master kill-switch)
 *   blocked_states                 : string[] (RMG-restricted states)
 *   platform_commission_percent    : number
 *   min_entry_fee_paise / max_entry_fee_paise
 *   score_confirmation_window_mins : number   (default 1440)
 *   slot_hold_duration_seconds     : number   (default 300)
 *   challenge_match_window_mins    : number
 *   withdrawal_min_paise / withdrawal_daily_cap_paise
